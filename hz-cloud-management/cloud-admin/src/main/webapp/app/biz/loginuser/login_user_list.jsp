...
<html>
  <head>
	...
	<script type="text/javascript">
	         	...
	         	dataType:'json',
	          	success : function(msg) {
					...
					/* if(data.messageCode == "0026" ){
		                ...
		                $.messager.alert("信息提示", "用户数据导入成功!", "info",function(){
							window.location = "${app}/loginuser/toLoginUserListPage"
						});
		          		return false;
		          	}else{
		          		$.messager.show({
		...
		function load() {  
		    ...
		    $("<div class=\"datagrid-mask-msg\"></div>").html("导入数据量比较庞大,时间较久，请耐心等待。。。").appendTo("body").css({ display: "block", left: ($(document.body).outerWidth(true) - 190) / 2, top: ($(window).height() - 45) / 2 });  
		}
		//取消加载层  
		function disLoad(){  
		...
		/* 根据手机号码查询用户是否存在 */
		function findLoginUserByUsername(){
			$("#msg_manager").html("");
			var usernameManager = $("#username_manager").val();
            if(validateTel(usernameManager)){
	            $.ajax({
	                url : "${app}/loginuser/getUserByUsername",
	                data : {"username":usernameManager},
	                type : "post",
	                dataType : "json",
	                success : function(data) {
	                    if(data.result == false){//查询用户成功
	                    	$("#msg_manager").html("该注册手机号已存在!");
	        		        $("#msg_manager").css("color","red");
	                    }
	                    if(data.result == "error"){
	                    	$("#msg_manager").html("查询汇中网失败,出现异常!");
	        		        $("#msg_manager").css("color","red");
	                    }
	                }
	            })
		...
		function validateTel (value){
			$("#msg_manager").html("");
			var validate = /^(1[3,4,5,7,8])\d{9}$/; //验证手机号
			if(validate.test(value)){
				return true;
			}else{
				$("#msg_manager").html("手机号格式不正确!");
		        $("#msg_manager").css("color","red");
				return false;
			}
		}
		function usernameNumber(){
			$("#msg_manager").html("");
			var username = $("#username_manager").val();
			validateTel(username);
		}
		//新增团队管理者弹框
		function openInsertManager(){
			$("#msg_manager").html("");
			$("#username_manager").val("");
			$("#insertUserManagerDiv").dialog('open');
		}
		//关闭团队管理者弹框
		function closeInsertManager(){
			$("#insertUserManagerDiv").dialog('close');
		}
		//新增团队管理者提交操作
		function submitInsertForm(){
			var username_manager = $("#username_manager").val();
			var msg = $("#msg_manager").text();
     		if(validateTel(username_manager) && msg ==""){
				openMask();
				$('#insertUserManagerDiv').dialog('close');
				$("#insertUserManageForm").ajaxSubmit({
	          		type : 'POST',
	          		url:'${app}/loginuser/insertUserManager',
	         		dataType:'json',
	          		success : function(msg) {
	          			closeMask();
	          			parent.createTab('${app}/loginuser/toLoginUserListPage?messageCode=' + msg.messageCode,'用户数据管理');
		...
		//设置邀请分享类型
		function editInviteShareType(){
			var rows = datagrid.datagrid('getSelections');
			if(rows.length <= 0){
				$.messager.show({
                    title:'信息提示',
                    msg:"请选择需要设置邀请分享类型的用户!",
                    timeout:3000,
                    showType:'slide'
                });
				return;
				...
				for(var i=0;i < rows.length;i++){
					if(rows[i].user_role == <%=Constants.USER_ROLE_CUSTOMER%>){
						$.messager.alert("信息提示", "设置邀请分享类型的用户不能包含客户!", "info",function(){});
						return;
					}
					userIds += rows[i].user_id + ",";
				}
				userIds = userIds.substr(0, userIds.length-1);
				$('#editInviteShareTypeDiv').dialog('open');
	   			$("#inviteShareUserIds").val(userIds);
			}
		}
		
		//提交编辑用户邀请类型的表单
		function submitInviteShareTypeForm(){
			$('#editInviteShareTypeDiv').dialog('close');
			$("#editInviteShareTypeForm").ajaxSubmit({
	          	type : 'POST',
	          	url:'${app}/loginuser/editUserInviteType',
	         	dataType:'json',
	          	success : function(msg) {
	          		parent.createTab('${app}/loginuser/toLoginUserListPage?messageCode=' + msg.messageCode,'用户数据管理');
	          	}
		...
		//编辑用户邀请类型取消
		function resetInviteShareTypeForm(){
			$('#editInviteShareTypeDiv').dialog('close');
		}
		
		/* 查看用户手机号 */
		function toFindUsername(){
			var rows = datagrid.datagrid('getSelections');
			if(rows.length > 0){
				if(rows.length > 1){
					$.messager.show({
				...
				$("#loginUserUsername").html("");
				$("#superiorUsername").html("");
				$("#selectUsernameDiv").dialog('open');
				var userId = rows[0].user_id;
				$.ajax({
					async:false,
					type:'post',
					data : {"userId":userId},
					url:"${app}/loginuser/findLoginUserUsernameByUserId",
					dataType:'json',
					success:function(data){
						$("#loginUserUsername").html(data.username);//用户手机号
						$("#superiorUsername").html(data.superiorUsername);//上级合伙人手机号
					}
				});
			}else{
				$.messager.show({
  ...
  <body class="easyui-layout" fit="true" style="width: 100%;height: 100%;">
 	...
 	<sec:authorize ifAnyGranted='${ctrl.user_data_mgr_is_show}'>
  		<a class="easyui-linkbutton" data-options="iconCls:'icon-search',plain:true" onclick="toFindUsername();">查看敏感信息</a>
		...
		<img src="${app}/images/separator.jpg" style="vertical-align: middle; *margin-top: -4px">
		<a class="easyui-linkbutton" data-options="iconCls:'icon-ok',plain:true" onclick="editInviteShareType();">设置邀请类型</a>
		<img src="${app}/images/separator.jpg" style="vertical-align: middle; *margin-top: -4px">
		...
		<img src="${app}/images/separator.jpg" style="vertical-align: middle; *margin-top: -4px">
		<div class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="openInsertManager();">新增团队管理者</div>
		<img src="${app}/images/separator.jpg" style="vertical-align: middle; *margin-top: -4px">
		</sec:authorize>
	</div>
	...
	<!-- 导入功能表单 -->
	 	...
	 	<form id="fileForm" enctype="multipart/form-data" style="text-align: center;padding-top: 10%">
		 	...
		 	<tr>
		 		<td align="center">
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					<a class="easyui-linkbutton" iconCls="icon-cancel" onclick="resetFileForm();">取消</a></td>
		 	</tr>
		 	<!-- 团队管理者导入table
		 	<tr style="padding-top: 50px;">
				<td width="40px;"></td>
				<td width="60%"><input type="file" name="file" id="fileName"/> </td>
				<td><a class="easyui-linkbutton" onclick="resetFileForm();">模板下载</a></td>
			</tr>
			...
			<tr>
				<td></td>
				<td><a class="easyui-linkbutton" id="submitButton"  iconCls="icon-ok" onclick="upload();">上传</a></td>
				<td><a class="easyui-linkbutton" iconCls="icon-cancel" onclick="resetFileForm();">取消</a></td>
			</tr>
			 -->
			</table>
  		</form>
  	...
  	<!--新增团队管理者 -->
	<div id="insertUserManagerDiv" style="display: none; overflow: hidden;width:45%;height:60%;"class="easyui-dialog" title="新增团队管理者" data-options="iconCls:'icon-edit',closed:true" >
		<form id="insertUserManageForm" style="width:480px;height:300px;margin-left: 20%;" >
			<table>
				<tr style="display:block;margin: 20px">
					<td><span style="font-size: 16px">请填写新增团队管理者用户信息</span></td>
				</tr>
				<tr style="display:block;margin: 3px">
					<td>手机号 : </td>
					<td>
						<input id="username_manager" name="username_manager"  style="width: 160px;height: 24px;" onblur="findLoginUserByUsername();" oninput="usernameNumber();"/>
						<span id="msg_manager"></span>
					</td>
				</tr>
				<br><br>
				<br><br>
				<tr>
					<td colspan="2" align="center" style="padding-top: 10px">
						<a class="easyui-linkbutton" id="submitButton"  iconCls="icon-ok" onclick="submitInsertForm();">提交</a>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a class="easyui-linkbutton" iconCls="icon-cancel" onclick="resetInsertForm();">取消</a>
					</td>
				</tr>
			</table>
		</form>
	...
	<!-- 设置邀请分享类型 -->
	<div id="editInviteShareTypeDiv" class="easyui-dialog" title="设置邀请类型" data-options="iconCls:'icon-save',closed:true" style="width:350px;height:200px;padding:10px">
	 	<form id="editInviteShareTypeForm"  style="text-align: center;padding-top: 10%">
	 		<table border="0"  width="100%">
		 		<tr style="padding-top: 50px">
		 			<td width="7%"> *邀请分享类型:</td>
					<td width="7%">
						<input type="hidden" id="inviteShareUserIds" name="inviteShareUserIds">
						<select  id="inviteShareTypeName" name="inviteShareTypeName" class="easyui-combobox" style="width: 160px;height: 24px;">
							<option value="" selected="selected">全部</option>
							<c:forEach var="obj" items="${CodeMap['INVITE_SHARE_TYPE']}">
								<option value="${obj.key}">${obj.value}</option>
							</c:forEach>
						</select>
					</td>
				...
				<tr>
					<td colspan="2" align="center" style="padding-top: 10px">
						<a class="easyui-linkbutton" id="submitButton"  iconCls="icon-ok" onclick="submitInviteShareTypeForm();">提交</a>
						&nbsp;&nbsp;&nbsp;&nbsp;
						<a class="easyui-linkbutton" iconCls="icon-cancel" onclick="resetInviteShareTypeForm();">取消</a>
					</td>
				</tr>
			</table>
  		</form>
  	...
  	<!--查看用户敏感信息 -->
	<div id="selectUsernameDiv" style="display: none; overflow: hidden;width:35%;height:45%;"class="easyui-dialog" title="查看用户敏感信息" data-options="iconCls:'icon-search',closed:true" >
		<form id="selectUsernameForm" style="width:100%;">
			<table border="0" class="searchForm" style="width:100%;">
				<tr style="display:block;margin-top: 50px;">
				</tr>
				<tr>
					<td style="font-size: 14px;"align="right">用户手机号 : </td>
					<td><span id="loginUserUsername" style="font-size: 14px"></span></td>
				</tr>
				<tr></tr>
				<tr>
					<td style="font-size: 14px;width: 50%;" align="right" >上级合伙人手机号 : </td>
					<td><span id="superiorUsername" style="font-size: 14px"></span></td>
				</tr>
			</table>
		</form>
	</div>
